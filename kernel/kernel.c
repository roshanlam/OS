...
int term_history(){	
	...
	char **args=NULL;
	if(!fp)
		fprintf(stderr, RED "term: file not found" RESET "\n");
	else{
		putchar('\n');
		while((c = getc(fp)) != EOF){
			putchar(c);
...
int term_execute(char **args){
	...
	if(cpid == 0){	
		if(execvp(args[0], args) < 0)
			printf("term: command not found: %s\n", args[0]); 
		exit(EXIT_FAILURE);
...
int term_grep(char **args){
	...
	if(args[0] != NULL && strcmp(args[0], "grep") == 0){
		if(args[1] != NULL && args[2] != NULL){
			...
			fprintf(stderr, RED "term: grep requires two params, " ITALICS "PATTERN" RESET RED " and " RED ITALICS "FILE" RESET "\n");
		}
	}
	if(flag == 0)
...
int term_help(char **args){
	if(args[0] != NULL && strcmp(args[0], "help") == 0){
		fprintf(stderr,"\n------\n" 
				BOLD "\nTerm " RESET "is a basic unix terminal shell written purely in C developed by Roshan Lamichhane for DevOS.\n"
				"\nSupported Commands:\n1. cd\n2. exit\n3. help\n4. touch\n5. cat"
				"\n\n------\n\n");
	}
	return 1;
...
int term_cd(char **args){
	if(args[1] == NULL){
		fprintf(stderr, "%sterm: Please enter a path to cd%s\n", YELLOW, RESET);
	}
	else{
		if(chdir(args[1]) > 0){
			perror("term");
...
char **split_line(char *line){
	...
	if(!tokens){
		fprintf(stderr, "%sterm: Allocation error%s\n", RED, RESET);	
		exit(EXIT_FAILURE);
	}
...
char *read_line(){
	...
	while(1){
		...
		if (position >= buffsize){
			...
			if(!buffer){
				fprintf(stderr, "%sdterm: Allocation error%s\n", RED, RESET);
				exit(EXIT_FAILURE);
			}
		}
	}
